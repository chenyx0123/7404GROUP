...
from utils.evaluate_utils import get_predictions, scan_evaluate, hungarian_evaluate, get_predictions_scan
...
def main():
    ...
    for epoch in range(p['epochs']):
        ...
        out, features = get_predictions_scan(p, train_dataloader, model, return_features=True)
        ...
        predictions = get_predictions_scan(p, val_dataloader, model)
        ...
        acc.append(clustering_stats['ACC'])
        lo.append(lowest_loss)
    ...
    predictions, features = get_predictions_scan(p, val_dataloader, model, return_features=True)
