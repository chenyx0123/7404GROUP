...
def visualize_indices(indices, dataset, hungarian_match, config):
    ...
    import numpy as np
    import time
    dirname = config['train_db_name']+str(time.time())
    ...
    for c in range(indices.shape[0]) :
        ...
        for idx in indices_c:
            ...
            plt.savefig('./'+dirname+'/'+config['train_db_name']+'_'+str(c)+'_'+str(idx)+'.png', dpi=500, bbox_inches='tight')



    # List the contents of the directory
    contents = os.listdir(dirname)
    picture_paths =sorted(contents)

    f, axarr = plt.subplots(3,10, figsize=(48, 32))
    j=0
    i = 0
    while i < (len(picture_paths)):
        picture_path1 = picture_paths[i]
        picture_path2 = picture_paths[i+1]
        picture_path3 = picture_paths[i+2]

        axarr[0,j].imshow(Image.open(dirname + "/" + picture_path1))
        axarr[1,j].imshow(Image.open(dirname + "/" + picture_path2))
        axarr[2,j].imshow(Image.open(dirname + "/" + picture_path3))
        axarr[0,j].axis('off')
        axarr[1,j].axis('off')
        axarr[2,j].axis('off')
    ...
    plt.subplots_adjust(wspace=0, hspace=0, left=0, right=0.5, bottom=0, top=0.23)
    plt.savefig('./'+dirname+'/' +'top3.png')
