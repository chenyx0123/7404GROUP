...
@torch.no_grad()
def get_predictions_scan(p, dataloader, model, return_features=False):
...
def scan_evaluate(predictions):
    ...
    for head in predictions:
        ...
        # Total loss
        total_loss = - entropy_loss + consistency_loss + ce_loss
        
        output.append({'entropy': entropy_loss, 'consistency': consistency_loss, 'ce': ce_loss, 'total_loss': total_loss})
